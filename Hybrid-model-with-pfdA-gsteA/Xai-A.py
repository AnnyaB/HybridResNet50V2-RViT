...
# THIS IS THE FILE FOR XAI FOR THE HYBRID MODEL A WITH PFD-GSTE VARIANT A
...
from pathlib import Path  # robust path handling (OS-independent so that it can run on Windows/Mac/Linux)
...
#  Fixing the error No module named models when running from scripts/ 
...
from models.hybrid_model import HybridResNet50V2_RViT  # imports my PFDA-GSTEA hybrid model A


def preprocess_pil(img, mean, std):  # converting PIL image into a normalized torch tensor batch
    img = img.convert("RGB")  # ensuring 3 channels (RGB)
    x = torch.from_numpy(np.array(img)).float() / 255.0  # converting to float tensor in [0,1], shape (H,W,3)
...
def normalize_map(h):  # scaling any heatmap tensor into [0,1] for visualization
    h = h - h.min()  # shifting so minimum becomes 0
    h = h / (h.max() + 1e-8)  # scale so maximum becomes ~1 (eps avoids divide-by-zero)
    return h  # return normalized heatmap


def gradcam_pp_from_activations(A, grads):  # computing Grad-CAM++ heatmap given activations and gradients
    ...
    if grads is None:  # if gradients aren't present, Grad-CAM++ can't work
        raise RuntimeError(  # throwing a clear error to help debugging
...
# computing token relevance map from attention layers
    """
    ...
    for attn in attn_list:  # loop over transformer layers
        ...
        mats.append(a)  # storing this layer’s normalized attention

    R = mats[0]  # starting rollout product with first layer
    ...
    return r.reshape(ht, wt)  # returning token-grid heatmap (ht, wt)


def overlay_and_save(img_rgb, heat, out_path, title):  
    # helper: overlay heatmap onto image and save
    
    plt.figure(figsize=(5, 5))  # creating a new figure of fixed size
    plt.imshow(img_rgb)  # showing the base RGB image
...
def main():  
    ...
    args = ap.parse_args()  # parsing CLI args into args
    ...
    state = ckpt.get("model_state", None)  # trying preferred key for weights
    if state is None:  # if not found,
        state = ckpt.get("state_dict", None)  # trying alternate common key name
    ...
    # Capturing CNN/PFD feature map via hook (no xai['cnn_feat']) 
    ...
    if not hasattr(model, "pfd"):  # safety check: ensuring this model actually has a PFD module
    ...
    hook_handle.remove()  # removing hook so it doesn't keep capturing on future calls

    prob = torch.softmax(logits, dim=1).squeeze(0)  # converting logits -> probabilities, drop batch -> (C,)
    pred_idx = int(torch.argmax(prob).item())  # predicteding class index
    ...
    ).squeeze(0).squeeze(0)  # drop batch and channel -> (224,224)
    cam = cam.detach().cpu().numpy()  # move to CPU and convert to numpy for plotting

    # Attention rollout 
    if not isinstance(xai, dict):  # ensuring model returned a dictionary payload
    ...
    attn_tok = attention_rollout(attn_list)  # computing token relevance map (ht,wt)
    ...
    img_rgb = np.array(img)  # converting PIL image to numpy array (H,W,3) for plotting
    overlay_and_save(  # saving Grad-CAM++ overlay
        img_rgb, cam, out_dir / "xai_gradcampp.png",
        f"Grad-CAM++ (target={class_names[target]})"
    )
    overlay_and_save(  # saving attention rollout overlay
        img_rgb, attn_img, out_dir / "xai_attn_rollout.png",
        "Attention Rollout (token relevance)"
    )

    print("Prediction (single pass):")  # header line
    print(f"  class = {class_names[pred_idx]} (idx={pred_idx})")  # predicted class name and index
